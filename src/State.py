...
class State:
    def __init__(self, map_graph, date_operational_range: Date_operational_range, simulator_config: Simulator_config):
        ...
        if date_operational_range.end_hour < date_operational_range.start_hour:
            self.time_horizon = (((23-date_operational_range.start_hour)+1) * 60) * 60
            self.time_horizon += ((date_operational_range.end_hour+1) * 60) * 60
        else:
    ...
    def _log_picked_up_request_metrics(self, bus_index, request_index):
        ...
        if request_row["Requested Pickup Time"].hour < self.date_operational_range.start_hour:
            desired_pickup_time = (((((request_row["Requested Pickup Time"].hour + 24) - self.date_operational_range.start_hour) * 60) \
        ...
        if pickup_deviation < 0:
            ...
            print('requests index', request_index)
            print(pick_up_time)
    ...
    def update_state(self, bus_index: int, request_index: int, request_row, assignment_cost: int, new_routing_plan: Routing_plan | None):
